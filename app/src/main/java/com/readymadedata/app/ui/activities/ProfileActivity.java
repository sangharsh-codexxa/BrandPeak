...
import com.readymadedata.app.databinding.ActivityProfileBinding;
...
public class ProfileActivity extends AppCompatActivity {
    ...
    private void setData(UserLogin data) {
        ...
        if (data.user.isSubscribed) {
            try {
                Date startDate = df2.parse(data.user.planStartDate);
                Date endDate = df2.parse(data.user.planEndDate);
